...
- Keep the user-facing surface as a single Rust binary (`krw`) while the repo is split into `krx-cli` and `krx-core`.
...
| 2026-03-07 | Split the repo into `krx-cli` and `krx-core` crates | Keeps `clap` isolated from reusable runtime logic without changing the `krw` command |
